...
func init() {
	...
	selectorFlags.Uint64Var(&selectorOpts.last, "last", 0, fmt.Sprintf("Get last N flows stored in Hubble's buffer (default %d). When querying against Hubble Relay, this gets N flows per instance of Hubble connected to that Relay.", hubdefaults.FlowPrintCount))
...
func New(vp *viper.Viper) *cobra.Command {
	observe := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			logrus.WithField("request", req).Debug("Sending GetFlows request")

			// handle flows
			if err := handleFlows(ctx, client, req); err != nil {
				msg := err.Error()
